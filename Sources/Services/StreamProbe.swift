...
extension StreamProbe {
    ...
    private static func probeRangeQuery(url: URL, videoID: String) async {
        ...
        // ── rev.86 で直した点 ────────────────────────────────────
        //
        // 以前は「3 か所のどれか 1 つでも 200/206 なら使える」と
        // 判定していた (anyOK)。だが先頭 512KiB は Range ヘッダでも
        // 普通に通るので、これでは何も切り分けられていない。
        //
        // 実測 (2026-08-18):
        //   先頭=200(512KiB) / 1MiB跨ぎ=403(0KiB) / 1MiB以降=403(0KiB)
        // それでも「range= クエリ方式が使える。Range ヘッダから
        // 切り替える価値がある」と出ていた。誤った結論で、
        // 実際には切り替えても 1 MiB の壁は動かない。
        //
        // 知りたいのは「Range ヘッダでは取れない 1 MiB 以降が、
        // range= クエリなら取れるのか」だけなので、そこだけを見る。
        var beyondOK = false
        var headOK = false
        ...
        for (label, value) in ranges {
            ...
            do {
                ...
                // 本文が空の 200/206 は「通った」とは言えないので、
                // 実際にバイトが返ったことも条件に入れる。
                let ok = (code == 200 || code == 206) && !data.isEmpty
                if label == "先頭" { headOK = ok }
                if label == "1MiB跨ぎ" || label == "1MiB以降" {
                    beyondOK = beyondOK || ok
                }
            } catch {
        ...
        // 1 MiB 以降が取れたときだけ「切り替える価値がある」と言える。
        if beyondOK {
            EventLog.log(.network, videoID: videoID,
                         message: "範囲診断の結論: range= クエリなら 1 MiB 以降も取れる。"
                             + "Range ヘッダから切り替える価値がある")
        } else if headOK {
            EventLog.log(.resolveNG, videoID: videoID,
                         message: "範囲診断の結論: range= クエリでも 1 MiB 以降は取れない。"
                             + "先頭が通るのは Range ヘッダと同じで、切り替えても解決しない")
        } else {
            EventLog.log(.resolveNG, videoID: videoID,
                         message: "範囲診断の結論: range= クエリは先頭すら通らない。"
                             + "この URL では使えない")
        }
    }
