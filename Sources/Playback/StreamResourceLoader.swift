...
final class StreamResourceLoader: NSObject {
    ...
    /// 1 MiB 制限で行き詰まったことを再生側へ知らせる。
    ///
    /// ── なぜ必要か (rev.87) ──────────────────────────────────
    ///
    /// SABR は `PlayerManager` の catch ブロックからしか呼ばれておらず、
    /// 「**最初の URL 解決が throw したとき**」だけが条件だった。
    ///
    /// ところが実測 (2026-08-18 のログ) で起きているのは:
    ///
    ///   1. ANDROID_VR が URL を返す        → 解決は成功
    ///   2. 再生が始まる                     → catch に入らない
    ///   3. offset 638976 付近で HTTP 403    → ここで詰む
    ///   4. AVPlayer は「曲が終わった」と判断 → 次の曲へ
    ///
    /// という流れで、SABR に一度も到達していなかった
    /// (`SABR 準備完了` がログに 0 回)。
    ///
    /// IOS / WEB は `adaptiveFormats` に url を持たない SABR 専用応答を
    /// 返すため、直接 URL を返すのは ANDROID_VR だけ。その ANDROID_VR が
    /// 1 MiB で切られる以上、SABR に逃げられないと詰みになる。
    ///
    /// そこで「1 MiB 制限と判断できる 403 で、URL の取り直しも
    /// クライアントの切り替えも尽きた」ときに、再生側へ知らせる。
    private let onBlockedByQuota: (@Sendable () -> Void)?

    /// 上のコールバックを既に呼んだか。1 曲につき 1 回だけ通知する。
    private var didNotifyQuota = false
    ...
    init(stream: StreamInfo,
         ...
         onBlockedByQuota: (@Sendable () -> Void)? = nil) {
        self.stream = stream
        self.videoID = videoID
        self.refresh = refresh
        self.onBlockedByQuota = onBlockedByQuota
    }
    ...
    private func fetch(from: Int, to: Int) async throws -> Data {
        ...
        for attempt in 0...2 {
            ...
            guard attempt < 2, let refresh else {
                notifyQuotaIfNeeded(status: status, from: from, to: to)
            ...
            // 従来は「制限は訪問者アイデンティティ単位でかかっており、
            // visitorData を作り直すと当たり外れが変わる」と考えていた。
            //
            // rev.85 でこの前提を見直している。公式 iOS アプリの
            // データコンテナを 3 回取得して比較したところ、公式は
            // visitorData をまったく引き直さずに itag 140 を
            // 3.5〜4.1 MiB 取得できていた。そのため既定では
            // `InnerTube.allowVisitorDataRenewal = false` にしてあり、
            // ここでの引き直しは通常スキップされる。
            //
        ...
        notifyQuotaIfNeeded(status: -1, from: from, to: to)
        throw StreamFetchError.badStatus(-1, offset: from)
    }

    /// 1 MiB 制限による行き詰まりかを判定し、再生側へ知らせる。
    ///
    /// 判定の条件は 2 つとも満たすこと:
    ///
    ///   - HTTP 403 (または再試行を使い切った -1) であること
    ///   - 要求範囲が 1 MiB 境界に掛かっていること
    ///
    /// googlevideo の制限は「ファイル先頭からの累積 1 MiB」で、
    /// 境界を跨ぐ要求はまるごと拒否される (部分的に返ってこない)。
    /// そのため `to`(終端) が 1 MiB 以上かどうかで判定できる。
    ///
    /// 期限切れや一時的な 403 と混同しないよう、範囲の条件を必ず見る。
    /// 先頭付近の 403 は URL の失効が原因のことが多く、SABR に
    /// 逃げる必要は無い (取り直しで通る)。
    private func notifyQuotaIfNeeded(status: Int, from: Int, to: Int) {
        guard !didNotifyQuota else { return }
        guard let onBlockedByQuota else { return }
        guard status == 403 || status == -1 else { return }
        guard to >= Self.quotaBoundary || from >= Self.quotaBoundary else { return }

        didNotifyQuota = true
        EventLog.log(.resolveNG, videoID: videoID,
                     message: "1 MiB 制限で行き詰まった "
                         + "(bytes=\(from)-\(to) / HTTP \(status))。"
                         + "SABR への切り替えを要求する")
        onBlockedByQuota()
    }

    /// googlevideo が拒否し始める累積バイト数。
    private static let quotaBoundary = 1_048_576
