...
final class PlayerManager: ObservableObject {
    ...
    private func loadCurrent() async {
        ...
        do {
            ...
            replaceStreamItem(stream: stream, song: song, token: token)
    ...
    private func replaceStreamItem(stream: StreamInfo, song: Song, token: Int) {
        ...
        let loader = StreamResourceLoader(
            ...
            onBlockedByQuota: { [weak self] in
                    // ── rev.87: 再生中の 1 MiB 制限から SABR へ ─────────
                    //
                    // このコールバックは AVFoundation のローダキュー
                    // (loaderQueue) から呼ばれるので、必ず MainActor に
                    // 戻してから PlayerManager の状態に触る。
                    //
                    // 捕捉するのは videoID (String) と token (Int) だけに
                    // する。Song をそのまま持たせると @Sendable クロージャの
                    // 制約に引っかかりうるので、曲は MainActor 側で
                    // currentSong から引き直す。
                    Task { @MainActor in
                        await self?.escalateToSABR(videoID: videoID, token: token)
                    }
                }
        )
    ...
    /// 再生中に 1 MiB 制限で行き詰まったとき、SABR へ切り替える。
    ///
    /// ── なぜ必要か (rev.87) ──────────────────────────────────
    ///
    /// SABR は元々 `loadAndPlay` の catch からしか呼ばれておらず、
    /// 「最初の URL 解決が throw したとき」だけが条件だった。
    ///
    /// だが実測 (2026-08-18) で起きているのは次の流れで、
    /// catch には一度も入らない:
    ///
    ///   ANDROID_VR が URL を返す → 再生開始 → 638976 付近で 403
    ///   → AVPlayer が「曲の終端」と判断して次の曲へ
    ///
    /// 結果 SABR は 1 度も実行されず (`SABR 準備完了` が 0 回)、
    /// 7 曲すべて途中で終わっていた。
    ///
    /// IOS / WEB は url を持たない SABR 専用応答しか返さないので、
    /// 直接 URL を返すのは ANDROID_VR だけ。その ANDROID_VR が
    /// 1 MiB で切られる以上、ここで SABR に逃げないと詰みになる。
    ///
    /// - Note: `StreamResourceLoader` から 1 曲につき 1 回だけ呼ばれる。
    ///   同じ曲で何度も走らないよう、こちらでも二重起動を防ぐ。
    private func escalateToSABR(videoID: String, token: Int) async {
        // 別の曲へ移っていたら何もしない。
        guard token == loadToken,
              let song = currentSong,
              song.id == videoID else {
            EventLog.log(.queue, videoID: videoID,
                         message: "1 MiB 制限の通知が届いたが、既に別の曲へ遷移済み")
            return
        }
        // 同じ曲で二重に走らせない。
        guard !didEscalateToSABR else { return }
        didEscalateToSABR = true

        EventLog.log(.playStart, videoID: song.id,
                     message: "再生中に 1 MiB 制限に当たった。SABR へ切り替える")

        // 再生位置を控えておく。SABR は曲全体を取り切ってから
        // 差し替えるので、聴いていた位置から続けられるようにする。
        let resumeAt = player.currentTime()
        let wasPlaying = isPlaying

        if await playViaSABR(song: song, token: token, started: Date()) {
            // 取得できたので、聴いていた位置まで戻す。
            //
            // 1 MiB で止まった位置より手前に戻すと聴き直しになるが、
            // 音が途切れるより自然なので、そのまま元の位置へ送る。
            if resumeAt.isValid, resumeAt.seconds > 1 {
                _ = await player.seek(to: resumeAt,
                                      toleranceBefore: .zero,
                                      toleranceAfter: .positiveInfinity)
                EventLog.log(.playStart, videoID: song.id,
                             message: "SABR 切り替え後 \(Int(resumeAt.seconds))秒 から再開")
            }
            if !wasPlaying {
                player.pause()
                isPlaying = false
        ...
        EventLog.log(.playError, videoID: song.id,
                     message: "SABR への切り替えも失敗した")
    }

    /// この曲で既に SABR へ切り替えたか。曲が変わるたびに false に戻す。
    private var didEscalateToSABR = false
