...
final class PlayerManager: ObservableObject {
    ...
    private func loadCurrent() async {
        ...
        do {
            ...
            guard let url = Self.playbackURL(for: stream) else {
                throw InnerTubeError.noStream(videoID: song.id)
            }
            replaceItem(url: url, song: song,
                        mimeType: stream.mimeType, clientName: stream.clientName)
    ...
    /// AVPlayerItem を差し替える。
    /// - Parameters:
    ///   - mimeType: ストリームの MIME (ローカルファイルなら nil)。
    ///   - clientName: URL を解決したクライアント名。UA を合わせるのに使う。
    private func replaceItem(url: URL,
        ...
        if let mimeType {
            // --------------------------------------------------------------
            // googlevideo の URL は "videoplayback?..." で拡張子が無いため、
            // AVFoundation がコンテナ形式を推測できず "unknown error" で
            // 失敗することがある。MIME を明示して判別させる。
            // --------------------------------------------------------------
            options["AVURLAssetOutOfBandMIMETypeKey"] = mimeType

            // 本家は ExoPlayer 側で特別な UA を設定していない。
            // 実測でも UA の一致は不要だったため、無難な Web の UA を渡す。
            _ = clientName
