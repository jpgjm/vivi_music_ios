...
struct PlayerView: View {
    ...
    /// アーティスト / アルバムのページへ進む経路。
    ///
    /// この画面は fullScreenCover で出ているため、タブ側の
    /// NavigationStack には積めない。ここに自前の階層を持たせて、
    /// 再生画面の中で開く (閉じれば元の再生画面に戻る)。
    ...
    var body: some View {
        NavigationStack(path: $browsePath) {
            playerBody
                // 再生画面そのものには自前のヘッダがあるので、
                // NavigationStack の余計なバーは隠す。
                // 進んだ先 (BrowseDetailView) では通常どおり出る。
                .toolbar(.hidden, for: .navigationBar)
                .navigationDestination(for: BrowseRoute.self) { route in
                    BrowseDetailView(route: route) { browsePath.append($0) }
                }
        }
    }
    ...
    private var background: some View {
        ZStack {
            ...
            if let url = player.currentSong?.thumbnailURL, let parsed = URL(string: url) {
                // ここも Artwork と同じ理由で Color.clear に敷く。
                // 直接 scaledToFill すると横長サムネイルのときに
                // 画像の縦横比がそのまま ZStack のサイズになり、
                // 背景が画面幅を超えて広がってしまう。
                Color.clear
                    .overlay {
    ...
    private var header: some View {
        HStack {
            ...
            if together.state.isInRoom {
                Label(together.isHost ? "ホスト" : "同期中",
                      systemImage: "person.2.fill")
                    .font(.caption2)
                    .foregroundStyle(.green)
            } else if player.isPlayingLocal {
            ...
            Button {
                ...
                Image(systemName: "person.2")
                    .font(.body)
                    .foregroundStyle(together.state.isInRoom ? .green : .primary)
            }
            ...
            Menu {
                ...
                // アーティスト / アルバムへ進む。
                // browseId が分かっている曲だけ出す (分からない曲では
                // 押しても行き先が無いので、項目自体を出さない)。
                if player.currentSong?.artistRoute != nil
                    || player.currentSong?.albumRoute != nil {
                    Divider()
                    if let route = player.currentSong?.artistRoute {
                        Button {
                            browsePath.append(route)
                        } label: {
                            Label("アーティストを表示", systemImage: "person.crop.square")
                        }
                    }
                    if let route = player.currentSong?.albumRoute {
                        Button {
                            browsePath.append(route)
                        } label: {
                            Label("アルバムを表示", systemImage: "smallcircle.filled.circle")
                        }
                    }
                }
    ...
    private var artworkPane: some View {
        VStack {
            Spacer(minLength: 0)
            // 正方形に切らず、元の縦横比のまま出す。
            // MV やライブ映像のサムネイルは 16:9 が多く、
            // 正方形にトリミングすると中央しか見えなくなる。
            //
            // 一覧用の 544px を iPad の全画面で引き伸ばすと粗く見えるので、
            // ここだけ高解像度版 (最大 1280px) を取りに行く。
            // 高解像度版が無い動画では fallbackURL で元の URL に戻る。
            Artwork(url: ThumbnailURL.highResolution(player.currentSong?.thumbnailURL),
                    size: nil,
                    radius: Theme.largeArtworkRadius,
                    contentMode: .fit,
                    fallbackURL: player.currentSong?.thumbnailURL)
        ...
        VStack(spacing: 12) {
            ...
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    // アーティスト名は押すとアーティストページへ進む。
                    // 本家 Android 版も、この位置の名前を押すと
                    // artist/{id} へ遷移する作りになっている。
                    // browseId が分からない曲では、ただの文字として出す。
                    if let route = player.currentSong?.artistRoute {
                        Button {
                            browsePath.append(route)
                        } label: {
