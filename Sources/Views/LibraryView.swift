...
struct LibraryView: View {
    ...
    /// プレイリスト (UUID) と アルバム・アーティスト (BrowseRoute) の
    /// 2 種類を積むので、型を混ぜられる NavigationPath を使う。
    ...
    var body: some View {
        NavigationStack(path: $path) {
            ...
            .navigationDestination(for: UUID.self) { id in
                PlaylistDetailView(playlistID: id) { path.append($0) }
            }
            // 曲メニューの「アーティストを表示」「アルバムを表示」から
            // ここへ飛んでくる。渡し忘れると押しても無反応になる。
    ...
    private var songList: some View {
        List {
            ForEach(songs) { song in
                SongRow(song: song,
                        showDownloadBadge: tab != .downloads,
