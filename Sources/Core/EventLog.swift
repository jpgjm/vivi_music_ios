...
enum EventLog {
    ...
    ///            └─ player-response/          (診断トグルが on のときだけ)
    ///                 └─ 20260813-044935.123_ANDROID_VR_xxxxx.json
    ///
    ...
    static func writeExportBundle() -> URL? {
        ...
        do {
            ...
            // 保存済みの player 応答があれば一緒に入れる。
            // 診断トグルがオフなら何も無いので、その場合は何もしない。
            if let dumps = PlayerResponseDump.directory,
               fm.fileExists(atPath: dumps.path) {
                try fm.copyItem(
                    at: dumps,
                    to: folder.appendingPathComponent("player-response", isDirectory: true))
            }
        } catch {
