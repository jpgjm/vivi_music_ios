...
struct JSON {
    ...
    /// `thumbnails` の中から一番大きいものを選ぶ。
    ///
    /// 幅だけで比べると、縦横比の違う候補が混ざったときに取り違える。
    /// 面積で比べるほうが確実 (Opaline も同じ理由で面積比較にしている)。
    var bestThumbnailURL: String? {
        ...
        let best = thumbs.max { a, b in
            let areaA = (a["width"].int ?? 0) * (a["height"].int ?? 0)
            let areaB = (b["width"].int ?? 0) * (b["height"].int ?? 0)
            return areaA < areaB
        }
        return best?["url"].string ?? thumbs.last?["url"].string
    }
}
