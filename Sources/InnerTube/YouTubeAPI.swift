...
enum YouTubeAPI {
    ...
    /// 検索フィルタ。値はオリジナル VIVI Music の SearchFilter から移植。
    enum SearchFilter: String {
        case song            = "EgWKAQIIAWoKEAkQBRAKEAMQBA%3D%3D"
        case video           = "EgWKAQIQAWoKEAkQChAFEAMQBA%3D%3D"
        case album           = "EgWKAQIYAWoKEAkQChAFEAMQBA%3D%3D"
        case artist          = "EgWKAQIgAWoKEAkQChAFEAMQBA%3D%3D"
        case featuredPlaylist = "EgeKAQQoADgBagwQDhAKEAMQBRAJEAQ%3D"
    }
    ...
    static func searchSongs(_ query: String) async throws -> [Song] {
        let started = Date()
        let json = try await InnerTube.shared.search(query: query,
                                                     params: SearchFilter.song.rawValue)
        var songs = Parsers.searchSongs(json)

        // 曲フィルタで 0 件なら動画フィルタでもう一度試す
        // (YouTube Music に無い曲でも YouTube 側にはあることが多い)
        if songs.isEmpty {
            EventLog.log(.search, message: "曲フィルタで 0 件 → 動画フィルタで再検索")
            let videoJSON = try await InnerTube.shared.search(query: query,
                                                             params: SearchFilter.video.rawValue)
            songs = Parsers.searchSongs(videoJSON)
        }

        EventLog.logDuration(.search, start: started,
                             message: "\"\(query)\" → \(songs.count) 件")
        return songs
    }
