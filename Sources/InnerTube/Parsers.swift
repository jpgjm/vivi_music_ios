...
enum Parsers {
    ...
    private static func meaningfulRuns(_ node: JSON) -> [JSON] {
    ...
    /// runs を " • " で連結した文字列にする。
    private static func subtitleText(_ node: JSON) -> String? {
    ...
    private static func thumbnail(_ renderer: JSON) -> String? {
    ...
    private static func upgradeThumbnail(_ url: String) -> String {
    ...
    static func song(fromResponsiveListItem renderer: JSON) -> Song? {
        ...
        // アルバム名 (browseId が MPREb_ で始まる run)
        let albumName = runs.first {
            $0["navigationEndpoint"]["browseEndpoint"]["browseId"].string?.hasPrefix("MPREb") == true
        }?["text"].string
