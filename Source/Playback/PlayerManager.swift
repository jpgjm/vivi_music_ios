...
final class PlayerManager: ObservableObject {
    ...
    /// イコライザー / 音響効果 (DSP)。曲ごとに取り付け直す。
    private var dspTap: DSPTap?
    ...
    // MARK: - イコライザー / 音響効果 (DSP)

    /// 再生中の音声に DSP を取り付ける。
    ///
    /// AVPlayer では AVAudioEngine の effect chain を直接挟めないので、
    /// MTAudioProcessingTap で音声を受け取り、DSPEngine の chain に通す。
    /// 音声トラックの取得は非同期なので、取り付けは少し遅れて行われる。
    /// 設定の変更は DSPEngine が DSPSettings を購読して反映する。
    private func attachEqualizer(to item: AVPlayerItem, asset: AVAsset) {
        // 前の曲のリバーブの残響やフィルタの履歴が残っていると、切り替わり際に雑音が出る
        DSPEngine.shared.resetEffectState()
        ...
        Task { @MainActor in
            do {
                ...
                if let mix = tap.makeAudioMix(track: track) {
                    item.audioMix = mix
                }
            } catch {
                EventLog.logError(.dspError, error: error, context: "DSP の取り付け")
            }
        }
    }
