...
/// 検索結果 1 ページ分。
///
/// InnerTube の検索も、ホームと同じく一度に全件は返さない。
/// 20 件ほど返したうえで `continuation` トークンを添えてくるので、
/// 下までスクロールしたらそれを使って続きを取りにいく。
/// 本家 VIVI Music の `SearchResult` にあたる。
struct SearchResultPage {
    var sections: [SearchSection]
    /// 次のページを取るためのトークン。nil なら終端。
    var continuation: String?

    static let empty = SearchResultPage(sections: [], continuation: nil)
}
