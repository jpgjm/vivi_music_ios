...
enum YouTubeAPI {
    ...
    ///
    /// Opaline は 5 回。健全な割合が 25〜35% なら、
    /// 5 回引けば 8〜9 割は当たる計算になる。
    /// 一方で 1 回ごとに player 要求が走るので、
    /// 増やしすぎると再生開始が遅くなる。
    ...
    private static func resolveStreamOnce(videoID: String,
        ...
        // 2026-08-23 追記: VISIONOS を先頭に置く。
        //
        //   これまでの並びは「どのクライアントなら player を通せるか」で
        //   決めていた。しかし本当の壁は player ではなく googlevideo 側の
        //   **先頭 1 MiB (音声で約 65 秒) で配信を打ち切る制限** だった。
        //   ANDROID_VR で URL が取れても、再生は必ず 1 分で止まる。
        //
        //   Opaline が 2026-08-18 に測った結果 (issue #76):
        //     android_vr / android / ios … 59904〜69888ms で打ち切り
        //     VISIONOS                   … 最後まで配信される
        //
        //   同じ測定を 2026-08-23 に再現した (dQw4w9WgXcQ / itag 140 /
        //   3449447B、512KiB ずつの Range 要求):
        //     VISIONOS        7 回とも 206。全長を取得
        //     ANDROID_VR 1.65 offset 1048576 (ちょうど 1 MiB) で 403
        //     IOS             adaptiveFormats に url が無い (SABR 専用)
        //
        //   つまり従来の「visitorData を引き直して健全な籤を待つ」策は、
        //   VISIONOS を使う限り不要になる (籤引き自体が起きない)。
        //   引き直しの仕組みは、VISIONOS が塞がれたときのために残す。
        //
        //   VISIONOS も匿名クライアントなので bot 判定 (LOGIN_REQUIRED) は
        //   受けうる。そのときは従来どおり下の候補に落ちる。
        //
        // 2026-08-23 追記 2: TVHTML5 をこの一覧から外した。
        //
        //   TVHTML5 の応答は SABR 形式で、adaptiveFormats に url も
        //   signatureCipher も入っていない。ここで拾えるのは muxed の
        //   itag 18 (AAC 96kbps + 360p 映像) だけで、しかもその URL は
        //   他の匿名クライアントと同じ 1 MiB 制限を受ける。
        //   つまり「TV で URL を取る」経路には価値が無い。
        //
        //   価値があるのは **TV + OAuth で SABR セッションを張る**ほうで、
        //   打ち切りを外せるのはそちらだけ (SABRStream.prepare を参照)。
        //   ここに TVHTML5 を残しておくと、muxed で解決に成功してしまい
        //   SABR の出番が来ない。だから外す。
        //
        //   TV は「全滅したときの逃げ道」として PlayerManager が
        //   SABR 経由で呼ぶ。位置づけは変わっていない。
        let allClients: [YouTubeClient] = [
            .visionOS,
            .androidVR165, .androidVR168, .androidVR143,
            .ios, .webRemix, .web,
        ]
        if signedIn {
            // ログイン中でも URL を取る経路は匿名クライアントのまま。
            // アカウントが効くのは SABR を張るときだけなので、
            // ここが全滅したあとに PlayerManager が TV で拾いに行く。
            EventLog.log(.resolveOK, videoID: videoID,
                         message: "ログイン中: 全滅したら TV + OAuth の SABR に落ちる")
        }
        ...
        for client in clients {
            do {
                ...
                // ── cver= を名乗る (Opaline の directURL と同じ) ──────
                //
                // Opaline は再生 URL に必ず `cver=<clientVersion>` を足す。
                // 「CDN が cver と URL を発行したクライアントを突き合わせる」
                // というのが向こうの注記。
                //
                // 2026-08-23 の実測では、付けても付けなくても 206 だった
                // (cver は sparams に無いので署名の対象外)。
                // 効果は確認できていないが、参照実装と素性を揃えておく。
                // 不要と分かったら、この if ごと消してよい。
                if client.clientName == "VISIONOS",
                   !StreamURL.hasParameter("cver", in: stream.url) {
                    let separator = stream.url.contains("?") ? "&" : "?"
                    stream.url = "\(stream.url)\(separator)cver=\(client.clientVersion)"
                }
